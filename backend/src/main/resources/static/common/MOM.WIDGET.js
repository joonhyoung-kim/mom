	...
	createChangePop: {
		password : function(index,title) {			
			var html  = '<div id ="changePwPop'+(index+1)+'"  class="modal modal-content modal-content-change-pop"style="display: none;height: 11rem;">'+
						 ...
						 '</div>'+
						 '<button id ="saveBtnCp'+(index+1)+'" class="btn btn-light" style="margin-top: 1.5rem;"><i class="mdi mdi-content-save-outline"></i>변경</button><button id ="closeBtnCp'+(index+1)+'" class="btn btn-light" style="margin-top: 1.5rem;margin-left: 0.5rem;"><i class="mdi mdi-window-close"></i>'+multiLang.transText('MESSAGE','MSG0005')+'</button>'+
						  '</div>'+
						'</div>';
			return html;
			}
			    ...
			    '<div class="modal-dialog" role="document">'+
			        '<div class="modal-content excelUploadPop">'+
			            ...
			            '<div class="modal-body">'+
			                ...
			                '</div>'+
			            '</div>'+
			            '<div class="modal-footer"><div class="excel-up-footer"><button class="btn excel-up-pop-btn " type="button" id="exUpCheck'+(index+1)+'"><i class="mdi mdi-file-find"></i>검사</button>  <button class="btn excel-up-pop-btn " type="button" id="exUpCheckDown'+(index+1)+'"><i class="mdi mdi-cloud-download"></i>검사결과</button>  <button class="btn excel-up-pop-btn " type="button" id="saveBtnExUp'+(index+1)+'"><i class="mdi mdi-file-upload-outline"></i>업로드</button> <button class="btn excel-up-pop-btn" type="button" id="cancelBtnExUp'+(index+1)+'"><i class="mdi mdi-window-close"></i>'+multiLang.transText('MESSAGE','MSG0005')+'</button></div</div>'+
			        '</div>'+
			    '</div>'+
			'</div>';
			return html;
        ...
        if(that.searchProperty[index] != undefined && jQuery.isEmptyObject(momWidget.searchProperty)==false){ 
            for(var i = 0, max = momWidget.searchProperty[index].length; i< max; i++){
  		      if(that.searchProperty[index][i]['columnRequire'] == 'Y' && param[momWidget.searchProperty[index][i]['searchId']] != undefined && param[momWidget.searchProperty[index][i]['searchId']] ==''){
  			      that.messageBox({type: 'warning', width: '400', height: '145', html: momWidget.searchProperty[index][i]['searchNm'] +' '+ multiLang.transText('MESSAGE','MSG0013')});
  			      return false;
  		      }										
...
/*	 				     successCount = 0;
		...
		//AUIGrid.showToastMessage(that.excelUpGrid[index], 2, 0, "Y/N만 입력가능!");
			...
			$(document).on('click', '#' + saveBtnId, function(e) {
				...
				callInitResult = that.checkActionCallInit(index, 'GS', param, 'saveBtn', your);
				if(callInitResult['result'] != 'SUCCESS') {
							  momWidget.messageBox({type:'danger', width:'400', height: '145', html: callInitResult['msg']});
							  momWidget.splashHide();
						      return;
		    ...
		    $.extend(item1, obj);               
				...
				for(var i=0,max2=popupCount;i<max2;i++){
					...
					if(that.popupProperty[index][i]['popupType'] == 'S'){
					...
					else if(that.popupProperty[index][i]['popupType'] == 'M'){
					...
					else if(that.popupProperty[index][i]['popupType'] == 'MS'){
					...
					if(containsYn == 'N'){
						...
						else if (actionType == 'CP' && that.popupProperty[index][i]['updateEditFlag'] == 'Y'&& that.popupProperty[index][i]['columnRequire']=='Y'){
						    if(fieldValue ==''){
							 momWidget.messageBox({type:'warning', width:'400', height: '145', html: popupItem[i]['popupNm'] +' '+ multiLang.transText('MESSAGE','MSG0013')});
							 momWidget.splashHide();
				             return;
			...
			$(document).on('click', '#' + editBtnId, function(e) {	
				...
				var popupTitle = that.gridExtraProperty[index]['popupTitle'];
				$('#popupTitle'+(index+1)).text('');
				$('#popupTitle'+(index+1)).append(popupTitle+multiLang.transText('MESSAGE','MSG0009'));	
				$('#' +'defaultPop'+(index+1)).momModal('show');
				that.htmlResize(index,your);
				//that.popUpSizeSet(index);		
				...
				modal : function() {
					var html = 
						...
						+'#{context}'
					   	...
					   	+'                    <div class="textblock">'+multiLang.transText('MESSAGE','MSG0004')+'</div>'
					   	+'                </a>'
						...
						if(actionToken == 'form') {
							...
							if(param <= 0) {
								setTimeout(function() {
									...
									if(that.searchFilter[index][param * -1]['sortIndex'] == undefined || that.searchFilter[index][param * -1]['sortIndex'] == '') {
										that.messageBox({type: 'warning', width: '400', height: '145', html: that.searchFilter[index][param * -1]['headerText'] +' '+ multiLang.transText('MESSAGE','MSG0013')});
										return;
									}
