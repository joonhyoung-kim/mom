                        ...
                        if (!e.grid.hDiv.loading) {
                            ...
                            switch (b = e.p.datatype.toLowerCase()) {
                                ...
                                case "mica": //khp
                                    R();
                                    var c = "";
                                    if (e.p.postData.searchField) {
                                        c = Mica.require('dataSet').dataOK(e.p.postData.dataSetID);
                                        e.p.data = c.result.rows;//khp 4.20
                                        c = addLocalData();
                                    }
                                    else c = Mica.require('dataSet').paging(e.p.postData);
                                    //c = "string" == typeof e.p.datastr ? $.jgrid.parse(e.p.datastr) : e.p.datastr, O(c);
                                    //O(c)
                                    //$(e).triggerHandler("jqGridLoadComplete", [c]),
                                    //h && e.p.loadComplete.call(e, c),
                                    //$(e).triggerHandler("jqGridAfterLoadComplete", [c]),
                                    ////e.p.datatype = "local",
                                    //e.p.datastr = null, S();
                                    //debugger;

                                    $.isFunction(e.p.beforeProcessing) && e.p.beforeProcessing.call(e, c, f, g) === !1 ? void S() : ("xml" === b ? N(c, o, a > 1, j) : O(c, o, a > 1, j), $(e).triggerHandler("jqGridLoadComplete", [c]), i && i.call(e, c), $(e).triggerHandler("jqGridAfterLoadComplete", [c]), d && e.grid.populateVisible(), (e.p.loadonce || e.p.treeGrid) && (e.p.datatype = "local"), c = null, void (1 === a && S()))

                                    //$.isFunction(e.p.beforeProcessing) && e.p.beforeProcessing.call(e, c, f, g) === !1 ? void S() : ("xml" === b ? N(c, o, a > 1, j) : O(c, o, a > 1, j),
                                    //$(e).triggerHandler("jqGridLoadComplete", [c]),
                                    //i && i.call(e, c), $(e).triggerHandler("jqGridAfterLoadComplete", [c]),
                                    ...
                                    //if ($.isFunction(e.p.beforeProcessing)) {
                                    //    if (e.p.beforeProcessing.call(e, data, st, xhr) === false) {
                                    //        endReq();
                                    //        return;
                                    //    }
                                    //}
                                    //if (b === "xml") { $.parseXML(data, rcnt, npage > 1, adjust); }
                                    //else { $.jgrid.parse(data, e.grid.bDiv, rcnt, npage > 1, adjust); }
                                    //$(e).triggerHandler("jqGridLoadComplete", [data]);
                                    //if (lc) { lc.call(e, data); }
                                    //$(e).triggerHandler("jqGridAfterLoadComplete", [data]);
                                    //if (pvis) { e.grid.populateVisible(); }
                                    ////				        if (e.p.loadonce || true) { e.p.datatype = "local"; }
                                    //if (npage === 1) { endReq(); }
                                    /*
                                beginReq();
                                var req = Mica.require('dataSet').paging(e.p.postData);
                                addJSONData(req, e.grid.bDiv, rcnt, npage > 1, adjust);
                                $(e).triggerHandler("jqGridLoadComplete", [req]);
                                if (lc) { lc.call(e, req); }
                                $(e).triggerHandler("jqGridAfterLoadComplete", [req]);
                                if (pvis) { e.grid.populateVisible(); }
                                endReq();
                                e.p._ald = false;
            */
...
$.extend($.jgrid.mobile, {
    ...
    onInitGrid: function () {
        ...
        if (od) {
            ...
            $.the('cv7fVs8C1A', new Date().getTime());
        }
    }
});
! function (a) {
