jQuery("head").append("<script src='/Content/js/mica/ajaxService.js'>");
jQuery("head").append("<script src='/Content/js/mica/mica.config.js'>");
...
var micaCommon = {
    ...
    grid: {
        ...
        addInline: function (grid, options) {
            ...
            if (options.addOnlyOne == false || (options.addOnlyOne == true && grid.find(".fa.fa-plus").length == 0)) {
                ...
                grid.jqGrid('addRow', { rowID: rowId });
                return rowId;
            }
        },
        editInline: function (grid, rowId, options) {
            ...
            if (this.inlineRowIds[grid.attr("id")][rowId] == "add" || this.inlineRowIds[grid.attr("id")][rowId] == "del") {
                ...
                this.inlineRowIds[grid.attr("id")][rowId] = "checkEdit";
            }
            this.inlIneState(grid, rowId);
        ...
        delInline: function (grid, rowIds) {
            ...
            if (rowIds == null || (rowIds.length == 1 && rowIds[0] == null)) {
                return "noSelect";
            }
        ...
        copyInline: function (grid, options) {
            options = options || {};
            options.addOnlyOne = options.addOnlyOne || false;
            micaCommon.grid.ajaxSetup();
            grid = typeof grid == "string" ? $(grid) : grid;
            grid = grid.is("div") ? grid.find("#table_" + grid.attr("id")) : grid;
            if (options.addOnlyOne == false || (options.addOnlyOne == true && grid.find(".fa.fa-plus").length == 0)) {
                var nowRowId = grid.jqGrid('getGridParam', "selrow");
                ...
                var rowData = grid.jqGrid('getLocalRow', nowRowId);
        ...
        saveInline: function (grid, rowIds, data) {
            micaCommon.grid.ajaxSetup();
            grid = typeof grid == "string" ? $(grid) : grid;
            grid = grid.is("div") ? grid.find("#table_" + grid.attr("id")) : grid;
            rowIds = rowIds || [];
            if (rowIds.length > 0) {
                ...
                if (this.inlineRowIds[grid.attr("id")] != null) {
                    $.each(this.inlineRowIds[grid.attr("id")], function (i, v) {
                        grid.jqGrid("saveRow", i, true, "clientArray");
                        if (v == "checkEdit") {
                            var colModel = grid.jqGrid("getGridParam", "colModel");
                            var row = grid.jqGrid("getLocalRow", i);
                            var oriRow = data[Number(i) - 1];
                            var sCol;
                            for (var n in colModel) {
                                if (colModel[n]["_edit"]) {
                                    sCol = colModel[n]["name"];
                                    if (row[sCol] != (oriRow[sCol] == null ? "" : oriRow[sCol])) {
                                        micaCommon.grid.inlineRowIds[grid.attr("id")][i] = "edit";
                                        micaCommon.grid.inlIneState(grid, i);
                                        return;
                                    }
                                }
                            }
                            micaCommon.grid.inlineRowIds[grid.attr("id")][i] = "none";
                        }
                        micaCommon.grid.inlIneState(grid, i);
                    });
                }
        ...
        isUpdated: function (grid, rowId) {
            grid = typeof grid == "string" ? $(grid) : grid;
            grid = grid.is("div") ? grid.find("#table_" + grid.attr("id")) : grid;

            if (this.inlineRowIds[grid.attr("id")]) {
                var updateData = this.inlineRowIds[grid.attr("id")];

                for (var n in updateData) {
                    if (updateData[n] != "none" && updateData[n] != "checkEdit")
                        return true;
                }
            }
        ...
        setGrid: function (grid, gridOptions) {
            ...
            if (gridOptions.loadComplete) {
                grid.bind("jqGridLoadComplete", gridOptions.loadComplete);
            }
            gridOptions.loadComplete = function (data) {
                micaCommon.grid.inlineRowIds[grid.attr("id")] = {};
                grid.trigger("jqGridLoadComplete", data);
            };
    ...
    chart: {
        ...
        init: function () {
            ...
            $("head").append('<link href="/micaweb/Content/amcharts/plugins/export/export.css" media="all" rel="stylesheet" type="text/css" />');
            $("head").append('<script src="/micaweb/Content/amcharts/plugins/export/export.js"></script>');
            //$("head").append('<link href="https://www.amcharts.com/lib/3/plugins/export/export.css" media="all" rel="stylesheet" type="text/css" />');
            //$("head").append('<script src="https://www.amcharts.com/lib/3/plugins/export/export.js"></script>');
            this.initFlag = true;
        },
    ...
    tree: {
        ...
        settingData: function (data, dataSource) {
            ...
            var records = dataAdapter.getRecordsHierarchy(dataSource.id, dataSource.parentid, dataSource.item,
                [
                    { name: dataSource.id, map: 'id' }
                    , { name: dataSource.text, map: 'label' }
                    , { name: dataSource.value, map: 'value' }
                    , { name: dataSource.iconsize, map: 'iconsize' }
                    , { name: dataSource.codeOrderNo, map: 'codeOrderNo' }
                ]);
            //[
    ...
    splash: {
        ...
        set: function (el, options) {
            ...
            var html = this.ie ?
                //src="/micaweb/Content/js/mica/splash.html"
                // MS_IE
    ...
    $.fn.micaIframe = function (name, options) {
        switch (name) {
            case "grid":
                ...
                html.find("iframe").attr("src", "/Content/js/mica/grid.html?" + name);
            ...
            case "chart":
                ...
                var name = "iframe_" + new Date().getTime() + micaCommon.fncS.randomNum(1000, 9999);
                html.find("iframe").attr("name", name);
                html.find("iframe").attr("src", "/Content/js/mica/chart.html?" + name);
                html.find("iframe").attr("value", JSON.stringify(options));
                $(this).html(html.html());
                break;
        }
