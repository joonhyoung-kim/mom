            ...
            if (a) {
                ...
                if ("object" == e) {
                    var f = 0; t.call(this, a, function (a, b) {
                        ...
                        }), s.call(this), f > 0 && (this.timer = new R({ duration: f, context: this }), this.active = !0, b && (this.timer.complete = h)); var g = this, j = !1, l = {}; I(function () {
                            t.call(g, a,
                                function (a) {
                                    a.active && (j = !0, l[a.name] = a.nextStyle)
                                }), j && g.$el.css(l);
                            if (g.$el.hasClass("w-tab-pane")) {
                                jQuery(window).trigger("resize");
                                //2016.09.26 - mica tab content 활성화 안된 항목 resize 수정
//                                var iframes = jQuery(parent.document).find("iframe");
//                                for (var i = 0; i < iframes.length; i++) {
//                                    var iframe = jQuery(iframes[i]);
//                                    var temp = iframe.height();
//                                    if (iframe.closest(".mdi-content").length > 0) {
//                                        temp = iframe.parent().height();
//                                    }
//                                    iframe.height(temp + 1);
//                                    setTimeout(function () {
//                                        iframe.height(temp);
//                                    }, 100)
//                                }
                ...
                if (dataSetID != null) {
                    ...
                    if (micaConfig.mobile) {
                        ...
                        micaCommon.comboGridBox.set($selectEL, { selectedIndex: 0, width: width, height: height, dropDownHeight: itemCount * ($($selectEL).height() < 18 ? 32 : $($selectEL).height()) },
                            { local: local, textName: "text", valueName: "value", gridClass: $selectEL.attr("class") });
                    }
...
Mica.define('splitter', function ($, _) {
    ...
    api.resize = function (el, options) {
        //        var splitters = $(this).find(".w-row[splitter]");
        var splitters = el == null ? $(".w-row[splitter]") : $(el).find(".w-row[splitter]");
        ...
        var gridTables = $(".ui-jqgrid.ui-widget.ui-widget-content");
        var grids = $(".w-widget.w-widget-jqgrid");
        ...
        for (var i = 0; i < splitterEL.length; i++) {
            ...
            if (splittertype != "col") {
                ...
                for (var j = 0; j < children.length; j++) {
                    childrenWidth[j] = Number($(children).attr("class").replace(/w-col w-col-/gi, "").split(" ")[0]) / 12 * 100;
                    childrenWidth[j] -= 1;
                ...
                $(el).on("resize", function (event) {
                    Mica.require("splitter").resize(this);
...
Mica.define('dataSet', function ($, _) {
    ...
    function build(dataSet, dataSetID, returnData) {
        ...
        var loading = '<div id="splash" style="float: left; position: absolute; z-index: 999; top: 0px; width: 99%; height: 99%; background-color: white; opacity: 0.6; display: none;">'
               + '<img alt="" src="/micaweb/Content/theme/img/loading.gif" style="position: absolute; border: none; left: ' + ($(window).width() / 2 - 230) + 'px; top:' + ($(window).height() / 2 - 200) + 'px">'
               + '</div>';
...
Mica.define('dropdown', function ($, _) {
    ...
    api.ready = api.design = api.preview = init;
    api.getDropdown = getDropdown;
    
    api.dropdownList = dropdownList;
    // -----------------------------------
    // Private methods
    ...
    function getDropdown(targetId) {
    	var dropdown;
    	$.each(dropdownList, function(key, ele){
    		if(targetId == ele.el.get(0).id){
    			dropdown = ele;
    		}
    	});
